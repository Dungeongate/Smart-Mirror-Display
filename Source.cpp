...
int main(int, char**) {
	...
	while (!end_main_loop){
		...
		//iterate seconds since last quote if the current second is different to the second from the previous loop
		if (c.getTime() != last_second){
			last_second = c.getTime();
			seconds_since_last_quote++;
		}

		// Change the quote if it's due to be changed
		if (seconds_since_last_quote >= seconds_per_quote){
			...
			StringQuote.changeText(QueryResult);
